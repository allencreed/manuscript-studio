...
ipcMain.handle('hms:document.load', async (_e, manuscriptId: string) => documentService.load(manuscriptId));
ipcMain.handle('hms:document.import', async (_e, payload: any) => documentService.import(payload));
ipcMain.handle('hms:version.list', async (_e, manuscriptId: string) => versionService.list(manuscriptId));
